...
/**
 * Represents CategoryList records as resources.
 *
 * @RestResource (
 ...
 */
class CategorylistResource extends ResourceBase implements DependentPluginInterface {
  ...
  public function get($id) {
    ...
    foreach ($tree as $term) {
      $tax_id = $term->tid->getValue()[0]['value'];
      $child_level2 = $manager->loadTree('category', $tax_id, 2, TRUE);
      $child_terms = [];
      /** @var \Drupal\taxonomy\TermInterface $term */
      foreach ($child_level2 as $child) {
        $child_terms[] = [
          'id' => $child->tid->getValue()[0]['value'],
          'name' => $child->getName(),
        ];
      }
      $result[] = [
        'id' => $tax_id,
        'name' => $term->getName(),
        'child' => (empty($child_terms)) ? NULL: $child_terms
      ];
    }
  ...
  public function post($data) {
    ...
    $id = $this->dbConnection->insert('ds_rest_categorylist')
      ->fields($data)
      ->execute();

    $this->logger->notice('New categorylist record has been created.');
  ...
  public function delete($id) {
    ...
    $this->dbConnection->delete('ds_rest_categorylist')
      ->condition('id', $id)
      ->execute();

    $this->logger->notice('CategoryList record @id has been deleted.', ['@id' => $id]);
  ...
  protected function loadRecord($id) {
    $record = $this->dbConnection->query('SELECT * FROM {ds_rest_categorylist} WHERE id = :id', [':id' => $id])->fetchAssoc();
    if (!$record) {
  ...
  protected function updateRecord($id, array $record) {
    ...
    $this->dbConnection->update('ds_rest_categorylist')
      ->fields($record)
      ->condition('id', $id)
      ->execute();

    $this->logger->notice('CategoryList record @id has been updated.', ['@id' => $id]);
